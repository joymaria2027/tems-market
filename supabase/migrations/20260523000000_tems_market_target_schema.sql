-- ============================================================
-- TEMS MARKET — Drop Legacy Objects
-- ============================================================
...
DROP FUNCTION IF EXISTS public.increment_affiliate_clicks(text) CASCADE;
...
BEGIN
  ...
  -- Customers and affiliates can be active immediately, admins/vendors are pending approval/invite
  IF v_role IN ('customer', 'affiliate') THEN
    v_status := 'active';
  ELSE
    v_status := 'pending';
  END IF;
...
CREATE TABLE public.price_layers (
  ...
  created_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT admin_price_above_base CHECK (true)
);
...
-- Customer can insert own orders
...
-- RPC: update_own_role to allow users to choose/update their role securely
CREATE OR REPLACE FUNCTION public.update_own_role(new_role user_role)
RETURNS void AS $$
BEGIN
  -- Only allow setting to customer, vendor, or affiliate
  IF new_role NOT IN ('customer', 'vendor', 'affiliate') THEN
  ...
  SET 
    role = new_role,
    status = CASE WHEN new_role = 'vendor' THEN 'pending'::user_status ELSE 'active'::user_status END
  WHERE id = auth.uid();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
